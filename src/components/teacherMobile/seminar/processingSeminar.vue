  ...
  <div class="root" @click="closeMenu">
    ...
    <div class="main">
      ...
      <div class="head2">
        ...
        <span style="height: 25px;width:15%;visibility: hidden"></span>
      </div>
      <div class="main1">
      ...
      <div class="main2">
        ...
        <div style="width: 20%;height: 100%;background: #fff;">
          <div class="asking" v-for="(item,index) in askGroup" :id="askId(index)" @click="switchAsk(index)">{{item.group}}  {{item.name}}</div>
        </div>
      </div>
      <div class="main3">
  ...
  export default {
    ...
    methods:{
      ...
      switchGroup:function(id){
        let _this=this;
        if(parseInt(id)>_this.$data.showNumber){
          _this.$message({
            message:'该组还未展示哦',
            type:'warning'
          })
      ...
      getAllTeam:function(){
        ...
        this.$axios({
          ...
          for(var j=_this.$data.showGroup.length-1;j>=0;j--){
            if(_this.$data.showGroup[j].isPresent==='1') {
